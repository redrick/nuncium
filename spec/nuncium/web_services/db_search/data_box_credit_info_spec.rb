RSpec.describe Nuncium::WebServices::DbSearch::DataBoxCreditInfo do
  it_behaves_like 'web service'

  describe '#to_xml' do
    it 'expects correctly formatted xml request' do
      web_service = Nuncium::WebServices::DbSearch::DataBoxCreditInfo.new
  ...
  describe '#call' do
    it 'receives correct response from API' do
      web_service = Nuncium::WebServices::DbSearch::DataBoxCreditInfo
        .new(dbID: ::Nuncium.configuration.data_box)

      VCR.use_cassette 'web_services/db_search/data_box_credit_info' do
        result = web_service.call
        expect(result).to be_a(Nuncium::WebServices::DbSearch::Response)
        expect(result.status).to be_a(Nuncium::Responses::Db::Status)
